...
// 更新进度条时间
- (void)updateVideoSlider1:(float)currentPlayTime
{
...
{
    CMTime duration;
    if([_playerItem respondsToSelector:@selector(asset)]) {
        AVAsset *asset = [_playerItem asset];
        duration = asset.duration;
    } else {
        duration = _playerItem.duration;
    }
    self.controlView.timeLabel.text = [NSString stringWithFormat:@"%.0f:%.0f", currentPlayTime, CMTimeGetSeconds(duration)];
    self.controlView.playerSilder.trackValue = currentPlayTime / CMTimeGetSeconds(duration);
}

BOOL compareCMTimerStruct(CMTime t1, CMTime t2) {
    return t1.value==t2.value && t1.timescale==t2.timescale && t1.flags==t2.flags && t1.epoch==t2.epoch;
}
...
- (void)updateVideoBufferProgress:(NSTimeInterval)buffer
{
    ...
    if (compareCMTimerStruct(duration, kCMTimeIndefinite)) {
        NSLog(@"_playerItem.duration = kCMTimeIndefinite");
    } else {
